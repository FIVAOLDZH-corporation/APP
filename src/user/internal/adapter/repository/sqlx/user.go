...
func (r *SQLXUserRepository) GetUserByUsername(ctx context.Context, username string) (*entity.User, error) {
	var repoUser repository.User

	err := r.db.GetContext(ctx, &repoUser, "SELECT * FROM users WHERE username = $1", username)
	if err != nil {
		return nil, err
	}
...
func (r *SQLXUserRepository) GetUserByEmail(ctx context.Context, email string) (*entity.User, error) {
	var repoUser repository.User

	err := r.db.GetContext(ctx, &repoUser, "SELECT * FROM users WHERE email = $1", email)
	if err != nil {
		return nil, err
	}
